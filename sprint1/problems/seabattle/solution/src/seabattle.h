...
public:
    ...
    SeabattleField(State default_elem = State::UNKNOWN) {
        ...
        if (default_elem == State::SHIP) {
            weight_ = 20;
        } else if (default_elem == State::KILLED) {
            weight_ = 0;
        } else {
            weight_ = 20;
        }
...
private:
        ...
        const auto mark_cell_as_used = [&](size_t x, size_t y) {
            for (int dy = -1; dy <= 1; ++dy) {
                for (int dx = -1; dx <= 1; ++dx) {
                    if (x + dx < field_size && y + dy < field_size) {
    ...
    void MarkKillInDirection(size_t x, size_t y, int dx, int dy) {
        ...
        // Ищем границы корабля в этом направлении
        size_t x1 = x, y1 = y;
        while (x1 < field_size && y1 < field_size && Get(x1, y1) == State::KILLED) {
            x1 += dx;
            y1 += dy;
        }
        
        // Отмечаем пустыми клетки вокруг корабля
        for (size_t i = 0; i < field_size; ++i) {
            size_t cx = x + dx * i;
            size_t cy = y + dy * i;
            if (cx >= field_size || cy >= field_size || Get(cx, cy) != State::KILLED) {
                break;
            }
            mark_cell_empty(cx + dy, cy + dx);
            mark_cell_empty(cx - dy, cy - dx);
        }
    }
