...
void LoadRoads(model::Map& map, const boost::json::object& map_obj) {
    if (auto roads_ptr = map_obj.if_contains("roads")) {
        const auto& roads_array = roads_ptr->as_array();
...
void LoadBuildings(model::Map& map, const boost::json::object& map_obj) {
    if (auto buildings_ptr = map_obj.if_contains("buildings")) {
        const auto& buildings_array = buildings_ptr->as_array();
...
void LoadOffices(model::Map& map, const boost::json::object& map_obj) {
    if (auto offices_ptr = map_obj.if_contains("offices")) {
        const auto& offices_array = offices_ptr->as_array();
...
void LoadMapProperties(model::Map& map, const boost::json::object& map_obj) {
    if (auto dog_speed_ptr = map_obj.if_contains("dogSpeed")) {
        map.SetDogSpeed(dog_speed_ptr->as_double());
    }
    
    if (auto bag_capacity_ptr = map_obj.if_contains("bagCapacity")) {
        map.SetBagCapacity(bag_capacity_ptr->as_int64());
    }
}
...
model::Game LoadGame(const std::filesystem::path& json_path) {
    ...
    if (auto speed_ptr = json_obj.if_contains("defaultDogSpeed")) {
        default_dog_speed = speed_ptr->as_double();
    }
    
    size_t default_bag_capacity = 3;
    if (auto capacity_ptr = json_obj.if_contains("defaultBagCapacity")) {
        default_bag_capacity = capacity_ptr->as_int64();
    }
