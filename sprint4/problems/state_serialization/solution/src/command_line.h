...
[[nodiscard]] inline std::optional<Args> ParseCommandLine(int argc, const char* const argv[]) {
    ...
    desc.add_options()
        ...
        ("state-file", po::value(&args.state_file)->value_name("file"), "path to state file for saving/restoring")
        ("save-state-period", po::value(&args.save_state_period)->value_name("milliseconds"), "period for auto-saving state");
