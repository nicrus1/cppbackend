...
public:
    ...
    void AddToken(const Player& player, const Token& token) {
        player_tokens_.AddToken(player, token);
    }
    
    std::optional<Token> FindTokenByPlayerId(PlayerId player_id) const {
        return player_tokens_.FindTokenByPlayerId(player_id);
    }
